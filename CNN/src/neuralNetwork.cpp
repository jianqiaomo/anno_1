...
const char *layerTypeName(layerType ty) {
    switch (ty) {
        ...
        case layerType::NCONV: return "NCONV";
        case layerType::NCONV_MUL: return "NCONV_MUL";
        case layerType::NCONV_ADD: return "NCONV_ADD";
    }
    return "UNKNOWN";
}

void dumpLayerLog(const layeredCircuit &circuit, const string &filename = "../output/CNN_layers.log") {
    ...
    for (int i = 0; i < circuit.size; ++i) {
        ...
        layers_log << std::left
                   ...
                   << std::setw(7) << static_cast<int>(cur.fft_bit_length)
...
//    printLayerValues(pr);
    ...
    if(debug_print)
    {
        layers_log.open("../output/CNN_layers.log", std::ios::out | std::ios::trunc);
        layers_log << "before initSubset:" << std::endl;
        layers_log.close();
        dumpLayerLog(pr.C);
    }
    ...
    if(debug_print)
    {
        layers_log.open("../output/CNN_layers.log", std::ios::out | std::ios::app);
