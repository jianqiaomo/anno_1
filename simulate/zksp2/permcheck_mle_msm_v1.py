...
def get_permcheck_mle_data(num_vars_range, permcheck_mle_units_range, bitwidth_data, freq, available_bw, onchip_mle_size, gate_type="vanilla", assume_onchip_storage=False):
    ...
    for num_vars in num_vars_range:
        ...
        for permcheck_mle_units in permcheck_mle_units_range:
            ...
            # print(peak_mle_bw, total_data_transferred, overlapped_data_transferred)
            # print(nd_cycles + frac_mle_cycles + prod_mle_cycles)
            # print(full_permcheck_mle_cycles)
            ...
            if peak_mle_bw > available_bw:
                ...
                # in cases where the available_bw is just under actual bw, we
                if time_penalty < 0:
                    assert abs(result['bandwidth_GiB_per_s'] - available_bw) / available_bw < 0.01
                    time_penalty = 0
            ...
            if debug:
                print(f"Permcheck MLE units: {permcheck_mle_units}, Peak MLE BW: {peak_mle_bw}, Time penalty: {time_penalty}")
                print("total_mle_cycles",full_permcheck_mle_cycles)
                print("time_penalty",time_penalty)
                print("overlapped_data_transferred",overlapped_data_transferred)
                print("total_data_transferred",total_data_transferred)
                print("initial_cycles",start_time_prod_mle_out)
                print("msm_overlap_cycles",overlapped_transfer_duration)
                print("nd_cycles", nd_cycles)
                print("frac_mle_cycles", frac_mle_cycles)
                print("prod_mle_cycles", prod_mle_cycles)
                print()
            ...
            frac_mle_modmuls = result["frac_mle_num_mod_muls"]
            frac_mle_numregs = result["frac_mle_num_regs"]
            ...
            permcheck_mle_logic_area_breakdown = {
                ...
                "nd_modmul_area": nd_modmuls*modmul_area,
                "nd_modadd_area": nd_modadds*modadd_area,
                "nd_reg_area": nd_numregs*reg_area,
                "frac_mle_modmul_area": frac_mle_modmuls*modmul_area,
                "frac_mle_reg_area": frac_mle_numregs*reg_area,
                "modinv_area": mle_modinv_area,
                "nd_modmuls": nd_modmuls,
                "nd_modadds": nd_modadds,
                "nd_numregs": nd_numregs,
                "frac_mle_modmuls": frac_mle_modmuls,
                "frac_mle_numregs": frac_mle_numregs,
                "num_modinvs": num_modinvs,
