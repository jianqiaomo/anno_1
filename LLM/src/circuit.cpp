...
void layeredCircuit::initSubset() 
{
    ...
    for (int i = 1; i < size; ++i) 
    {
        ...
        bool has_pre_layer_v = false; // whether any gate in this layer uses input v from a previous layer
        for (auto &gate: cur.uni_gates) 
        {
            ...
            has_pre_layer_u |= (gate.lu != 0); // has u-inputs from "previous layer": true
        }
...
void layeredCircuit::init(u8 q_bit_size, i64 _layer_sz) 
{
    size = _layer_sz;
    circuit.resize(size);
}
