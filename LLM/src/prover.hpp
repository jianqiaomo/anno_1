...
class prover {
public:
    vector<int> fc_row,fc_col,fc_start_id, fc_input_row,fc_input_col,fc_input_id;
    vector<int> fc_real_row,fc_real_col,fc_real_input_row,fc_real_input_col; // padded
    ...
    void sumcheckInitPhase1(const F &relu_rou_0, bool DEBUG_PRINT=false, int squeeze_merge=1, const std::string& model_name="gpt2-small");
    void sumcheckInitPhase2(bool DEBUG_PRINT=false, int squeeze_merge=1, const std::string& model_name="gpt2-small");
    ...
    F Vres(const vector<F>::const_iterator &r, u32 output_size, i64 r_size,int layer_id,int start=0) ;
    ...
    void sumcheckLassoInit(const vector<F> &s_u, const vector<F> &s_v,const vector<vector<F>>& r_uu, const vector<vector<F>>& r_vv,
                           bool DEBUG_PRINT=false, int squeeze_merge=1, const std::string& model_name="gpt2-small");
    ...
    void commitInput(const vector<G1> &gens,int thread_n=1, bool disable=false);
    ...
    F getCirValue(uint16_t layer_id, const vector<u64> &ori, u64 u);
    u64 proof_size;
    vector<G> gens;
    Commit_return cc;
...
private:
    ...
    i64 round;          // step within a sumcheck
    i64 sumcheck_id;    // the level
