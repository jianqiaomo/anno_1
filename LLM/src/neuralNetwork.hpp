...
public:
    ...
    void create(prover &pr, bool merge, bool DEBUG_MERGE=false, std::string MODEL_NAME="gpt2-small", bool disable_commit=false);
    vector<int> layer_norm_w_c, layer_norm_w_e,layer_norm_b_c, layer_norm_b_e;
    vector<int> layer_norm_w_q_start,layer_norm_b_q_start;
    vector<int> layer_norm_c1, layer_norm_e1, layer_norm_c2, layer_norm_e2;
...
protected:
    ...
    void relu_checker_layer(layer &circuit, i64 &layer_id, int real_cn_out);
    ...
    void fullyConnLayer(layer &circuit, i64 &layer_id, i64 first_fc_id, i64 x_offset,int x_layer);

    static void printLayerInfo(const layer &circuit, i64 layer_id);

    void readBias(i64 first_bias_id);

    void readFconWeight(i64 first_fc_id,int real_r,int real_c,int id, bool DEBUG_disable_rand=false);
